...
def ai_model(movies_watched):
    ...
    model = genai.GenerativeModel(
        ...
        system_instruction="You are a highly intelligent engine designed to analyze movie preferences based on user inputs. The input will be a list of movie names that the user liked. Your task is to creatively analyze the input and recommend up to 5 film titles that best match the user's preferences.\n\nThe output should be a list of film titles (just the names of the films) that the user can later use to search for more details on MovieDB. Ensure that the recommended films align with the given preferences, drawing from a wide range of movie genres, languages, and time periods.\n\n",
    )
    ...
    response = chat_session.send_message({
        'parts': [
            {
                'text': json.dumps(movies_watched)
            }
        ]
    }).to_dict()
...
@api_v1.route('/films/smart-recommend', methods=['GET'], strict_slashes=False)
def films_smart():
    data = movies_watched
    if data is None:
